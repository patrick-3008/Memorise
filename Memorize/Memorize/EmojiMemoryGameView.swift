...
struct EmojiMemoryGameView: View {
    ...
    var body: some View {
        VStack {
            HStack {
                Spacer()
                Label("Score: \(Int(viewModel.showBouns()))", systemImage: "dollarsign.circle")
                Spacer()
                Label("Moves Left: \(Int(viewModel.showMoves()))", systemImage: "move.3d")
            ...
            ZStack {
                    ...
                    CardView(card: card)
                        .onTapGesture {
                            ...
                            withAnimation(.linear(duration:duration)) {
                                self.viewModel.chooseVM(card: card)
                        }
                    }
                    .padding(8)
                }
                .padding()
                .foregroundColor(.red)
                if viewModel.isOutOfMoves() {
                    VStack {
                        Text("You are out of moves!")
                            .padding()
...
struct ContentView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        game.chooseVM(card: game.cards[0])
